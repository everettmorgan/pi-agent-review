...
When the reviewer denies a call the agent believes the user wants, the agent can call the `request_user_approval` tool, which shows the full tool name and arguments in a confirmation dialog. If the user approves, a one-shot approval (unique nonce, ~10 minute expiry) is recorded as a session entry along with a description of the approved action. When the agent next calls that tool, the reviewer is shown the approved action and authorizes the call only if it matches that action's scope and intent — so the approval survives trivial input differences (dynamic dates, formatting) rather than requiring a byte-identical retry. The approval is consumed only on a terminal approve, so a transient reviewer failure or denial does not force the user to re-approve. The reviewer can still deny hard-safety violations (secrets, exfiltration, destructive actions) even with user approval. Direct user messages and structured `ask_user_question` answers are passed as trusted context but are not the authorization mechanism.
