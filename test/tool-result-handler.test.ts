...
function lastLog(sendMessage: ReturnType<typeof vi.fn>): string | undefined {
	const call = sendMessage.mock.calls.find(([message]) => (message as {customType: string}).customType === reviewLogEntryType);
	return (call?.[0] as {details: {message: string}} | undefined)?.details.message;
}
...
describe('createToolResultHandler', () => {
	...
	it('passes clean output through, records the assessment, logs it, and updates the footer tally', async () => {
		...
		expect(lastLog(sendMessage)).toContain('Output review cleared read');
	...
	it('blocks, logs, and stops when a leak is detected', async () => {
		...
		expect(lastLog(sendMessage)).toContain('aws-key');
